...
def test_parse_message_tags():
    ...
    assert parse_message_tags("url=;netsplit=tur,ty") == {
        "url": "",
        "netsplit": "tur,ty",
    }
...
def test_parse_message_params():
    ...
    assert parse_message_params("* LS :multi-prefix sasl") == [
        "*",
        "LS",
        "multi-prefix sasl",
    ]
    assert parse_message_params("REQ :sasl message-tags foo") == [
        "REQ",
...
def test_parse_message_source():
    ...
    assert parse_message_source("cherryh.freenode.net") == Source(
        "cherryh.freenode.net", "", "", "cherryh.freenode.net"
    )
...
def test_parse_capabilities_ls():
    ...
    assert parse_capabilities_ls(params) == {"multi-prefix": True}
...
def test_parse_supported():
    ...
    assert supported == {"ELIST": "CTU", "KNOCK": "", "MONITOR": "100"}
