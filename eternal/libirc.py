...
class IRCClient:
    ...
    def _process_message(self, msg: Message) -> List[Message]:
        ...
        if msg.command == "366":
            ...
            def _member_from_nick(nick: str) -> Member:
                ...
                prefixes = nick[:i]
                highest_prefix = get_highest_member_prefix(
                    self.member_prefixes, prefixes
                )
                nick = nick[i:]
            ...
            self.channels[channel].members = {m.user.source.nick: m for m in members}

            return [ChannelNamesEvent(channel=channel, nicks=nicks, **msg.__dict__)]
    ...
    def _process_ping_message(self, msg: Message):
        self.send_message_to_server(ClientMessage(command="PONG", params=msg.params))
        return []
    ...
    def _generate_away_events(
        self, msg: Message, nick: str, is_away: bool, away_message: Optional[str]
    ):
        ...
        for channel in self.channels.values():
            ...
            if member is not None:
                if is_away:
                    ...
                    rv.append(
                        BackFromAwayEvent(
                            channel=channel.name, user=member.user, **msg.__dict__
                        )
                    )
                ...
                if mode in self.member_prefixes:
                    ...
                    if is_add:
                        ...
                        if prefix not in member.prefixes:
                            ...
                            member.highest_prefix = get_highest_member_prefix(
                                self.member_prefixes, member.prefixes
                            )
                    else:
                    ...
                    rv.append(
                        ChannelNamesEvent(
                            channel=channel.name, nicks=[], **msg.__dict__
                        )
                    )
                else:
                    ...
                    rv.append(
                        ChannelModeEvent(
                            channel=channel.name, modes=channel.modes, **msg.__dict__
                        )
                    )
    ...
    def _process_324_message(self, msg: Message):
        ...
        modes = "".join(
            [
                mode
                for is_add, mode, arg in self._iter_modestring(
                    modestring, args, is_channel=True
                )
                if is_add is True
            ]
        )
    ...
    def _process_tagmsg_message(self, msg: Message):
        ...
        if "+typing" in msg.tags:
            try:
                ...
                logger.warning(
                    'Received unknown typing status "%s"', msg.tags["+typing"]
                )
                return []
            ...
            try:
                ...
                logger.warning(
                    'Received a typing status for an unknown target "%s"', channel_name
                )
                return []
            ...
            try:
                ...
                logger.warning(
                    'Received a typing status for an unknown member "%s" of "%s"',
                    msg.source.nick,
                    channel_name,
                )
                return []
    ...
    def _should_send_active_typing_update(self, member: Member) -> bool:
        ...
        if member.last_typing_update_at is None:
            logger.warning(
                'Inconsistent state, member "%s" should have "last_typing_update_at" set',
                self.nick,
            )
            return True
    ...
    def notify_typing_active(self, channel_name: str):
        ...
        try:
            ...
            logger.warning(
                'Nick of current user "%s" is not a member of channel "%s"',
                self.nick,
                channel_name,
            )
            return
    ...
    def sort_members_by_prefix(self, members: Iterable[Member]) -> List[Member]:
        ...
        return sorted(
            members,
            key=lambda m: (
                [str(member_prefixes_symbols.index(c)) for c in m.prefixes]
                or ["z"] + list(m.user.source.nick.lower())
            ),
        )
    ...
    def _hanshake_start(self):
        ...
        self.send_to_server(
            f'USER {self._config["user"]} 0 * :{self._config["real_name"]}'
        )
    ...
    def _handshake_authenticate_step_1(self):
        ...
        self._handshake_steps.append(
            (is_authenticate_plus_received, self._handshake_authenticate_step_2)
        )
    ...
    def _handshake_authenticate_step_2(self):
        ...
        self._handshake_steps.append(
            (is_903_received, self._handshake_negociate_capabilities)
        )
        sasl_config = self._config.get("sasl")
...
TAG_VALUE_ESCAPE = {r"\:": ";", r"\s": " ", r"\\": "\\", r"\r": "\r", r"\n": "\n"}
...
def get_highest_member_prefix(
    server_prefixes: Dict[str, str], member_prefixes: str
) -> str:
